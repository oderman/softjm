...
if ($_POST["idSql"] == 3) {
	...
	if ($numeroA == 0) {
		...
		while ($contador < $numero) {
			mysql_query("INSERT INTO paginas_perfiles(pper_pagina, pper_tipo_usuario)VALUES(" . $_POST["paginasP"][$contador] . ",'" . $idInsertU . "')", $conexion);
			if (mysql_errno() != 0) {
				echo informarErrorAlUsuario(__LINE__, mysql_error());
				exit();
			}
...
if ($_POST["idSql"] == 4) {
	...
	if ($numeroA == 0) {
		...
		while ($contador < $numero) {
			mysql_query("INSERT INTO paginas_perfiles(pper_pagina, pper_tipo_usuario)VALUES(" . $_POST["paginasP"][$contador] . ",'" . $_POST["id"] . "')", $conexion);
			if (mysql_errno() != 0) {
				echo informarErrorAlUsuario(__LINE__, mysql_error());
				exit();
			}
...
if ($_POST["idSql"] == 92) {
	...
	if($_POST['zonasTodas'] == 1){
		...
		$zonasConsulta = mysql_query("SELECT * FROM zonas",$conexion);
		while($zonasDatos = mysql_fetch_array($zonasConsulta)){

			mysql_query("INSERT INTO zonas_usuarios(zpu_usuario, zpu_zona)VALUES('" . $_POST["id"] . "'," . $zonasDatos[0] . ")", $conexion);
			if (mysql_errno() != 0) {
				echo informarErrorAlUsuario(__LINE__, mysql_error());
				exit();
			}
