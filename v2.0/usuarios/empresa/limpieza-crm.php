...
$clientes = mysqli_query($conexionBdPrincipal,"SELECT * FROM clientes WHERE cli_id_empresa='".$idEmpresa."'");
...
while($cliente = mysqli_fetch_array($clientes, MYSQLI_BOTH)){
	...
	$facturas = mysqli_num_rows(mysqli_query($conexionBdPrincipal,"SELECT * FROM facturacion WHERE fact_cliente='".$cliente['cli_id']."' AND fact_id_empresa='".$idEmpresa."' "));
	
	$remisiones = mysqli_num_rows(mysqli_query($conexionBdPrincipal,"SELECT * FROM remisiones WHERE rem_cliente='".$cliente['cli_id']."' AND rem_id_empresa='".$idEmpresa."'"));
